...
2. **Escopo do produto** (o QUE construir):
   - `ESCOPO-PRODUTO.md` — **IA canônica dos 4 módulos** (MAPA→PROVAS→COMPETIÇÕES→PILOTOS, Márcio 16/06) + gaps código×escopo. Manda no *escopo*; fica abaixo das calibrações de *scoring*.
3. **Estado atual** (descrevem o que está construído e calibrado):
...
4. **Especificações de produção** (construtor de mapas):
   - `CONSTRUTOR-PLANO.md` — plano do construtor por tipo de prova.
   - `ESPEC-PRODUCAO-MAPAS.md` — requisitos de mapa A3/A4.

> O briefing antigo (`BRIEFING-CLIENTE.md`) e o modelo de dados idealizado (`MODELO-DADOS-PROPOSTO.md`) foram **removidos** por conflitarem com as calibrações (Emax 180≠300, códigos `2.A1` de microleve) e com a IA atual. O escopo válido vive em `ESCOPO-PRODUTO.md`; as pendências de regra (P1–P6) e a penalidade de backtracking foram preservadas lá.

## Glossário (nomes antigos × projeto)

| Briefing antigo | Projeto (em produção) | Regulamento (paramotor, Part 3) |
|---|---|---|
...
| `CURVE` | **N3** · Navegação em Curva (corredor curvo, ⚠️ não-calibrado) | `3.A2` (corredor); `2.A1` é microleve (Part 2), não se aplica |
