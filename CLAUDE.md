...
- **`core/`** — núcleo portado e validado:
  ...
  - `models.py` — `Prova` (campos extra: `route` corredor N3, `landings` pousos), `Point`, `Pilot`; `Prova.mapdata()` gera o JSON do endpoint. `mappdf.py`/`pointspdf.py` geram os PDFs A3/A4.
...
- **N2 (Tempo declarado):** Total (máx 1000) = ScoreHG (máx 400) + ScoreTG (máx 200) + ScoreVel (máx 400).
