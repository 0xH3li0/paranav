...
Testes: **`validate.py`** = gate de regressão (invariantes geométricos sagrados N1/N2/N3, sem servidor); **`run_tests.py`** = suíte ampla (geo, parser, modelos, scoring, repos files+sqlite, rotas via test client, construtor, PDFs, Paranapanema); **`apurador/report.py`** = gerador do PDF de relatório de voo (`python -m apurador.report`). Rodar **`python3 validate.py`** (e `python3 run_tests.py`) antes/depois de mexer no scoring. Valores de referência (não regredir):
...
- **`core/models.py`** (único modelo de dados) — `Mapa` (geometria reutilizável) + `Prova` (`map_slug` + scoring) + `Point`/`Pilot`; `hydrate(prova, mapa)` copia a geometria. Áreas têm 3 cores: `kind` proibida(vermelho)/atencao(amarelo)/livre(verde). **Helpers compartilhados:** `core/slugs.py` (`slugify`/`slug_from_filename`) e `pdfcommon.py` (constantes/tiles/zoom dos geradores reportlab).
- **`core/`** — núcleo portado e validado:
  ...
  - `models.py` — detalhado acima (`Mapa`/`Prova`/`Point`/`Pilot`, `Prova.mapdata()` gera o JSON do endpoint). **PDFs (fora do `core/`):** **A3 = QGIS** (`qgis_render.py` PyQGIS headless + `qgispdf.py` wrapper subprocesso, estilo AITA; `mappdf.py` é **fallback** reportlab no dev/sem-QGIS), `pointspdf.py` = folha A4 de pontos, `report.py` = PDF de relatório de voo. Liga/desliga o QGIS por env `APURADOR_QGIS=1`.
...
- **`routes/`** — `auth.py` (login/logout por sessão), `api.py` (`/api/sala/<slug>/mapdata`, `/trackdata`, `/state/reset`), `main.py` (igcupload público, viewer, scores, competicoes, upload/tracks, mapas/editor, relatorio, **prova_config** = `/prova/<slug>/config`, **builder** = `/builder[/<slug>]` + `/builder/save`), **`pdf.py`** (downloads de PDF: mapa A3, folha A4 de pontos e relatório de voo — anexa-se ao **mesmo blueprint `main`**, então os endpoints/`url_for` não mudam).
- **Editor de Mapa** (`/mapas`, `static/js/mapa_editor.js`). **Referência de mapa = mapas AITA (QGIS), não o programa do Marcinho.** **Etapa 1** "Novo Mapa": nome, escala, **orientação (retrato/paisagem)**, base (topo/satélite/OSM), teto AMSL, alt. mínima, bloco **EVENTO** (organizador/título/data/local) + posicionar a **folha A3**. **"Criar mapa" CAPTURA a área** → trava o mapa na folha (`setMaxBounds`; **zoom livre p/ precisão**), escurece fora. **Etapa 2** (barra estilo Earth Pro): **Ponto · Polígono · Retângulo · Círculo · Rota · Desfazer · Refazer** (ferramentas em *toggle*; sem botão fixo de "Mover"/seleção) + **Salvar/Publicar/Preview**; painel **"Dados do Mapa"** (Estilo/Rodapé/Logo/Pontos(n)/Áreas(n)/Auto-calculados). Áreas/rota via **Terra Draw** (polígono/retângulo/círculo, cor por tipo verde/vermelho/amarelo); os pontos são geridos no editor (marcador arrastável + anel do raio). **Rotação livre ao posicionar** (terreno gira, folha A3 reta na tela capturando a faixa girada; ao "Criar mapa" o ângulo é travado). Rotas `/mapas/<slug>/mapa.pdf` (orientação respeitada, logo no rodapé) e `/pontos.pdf`. **A geometria mora aqui, não na prova.**
...
- **Deploy = VPS dedicado `ubuntu-vinhedo`** (NÃO Vercel/Supabase — app é stateful, serverless não encaixa). gunicorn + systemd + nginx (ou Cloudflare Tunnel), backend `sqlite`. Artefatos e passo a passo em `webapp/deploy/` (`README-deploy.md`, `aeronav.service`, `aeronav.nginx.conf`, `aeronav.env.example`). App escuta em **`172.19.0.1:8050`** (gateway da rede Docker `proxy`, alcançável pelo cloudflared, **não público**; use `127.0.0.1:8050` só se for com nginx no host — ver `APURADOR_BIND` em `webapp/deploy/aeronav.env.example`); **não derrubar** os serviços já ativos (gitea/redis/bind9/mediamtx).
